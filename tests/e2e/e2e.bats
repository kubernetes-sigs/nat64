...
@test "bpf metrics map works" {
  ...
  ip_address=$(ip addr show "$default_interface" | grep "inet " | awk '{print $2}' | cut -d/ -f1)
  # perform the ping
  kubectl run test-bpfmap1 --privileged --image registry.k8s.io/e2e-test-images/agnhost:2.39 \
    --restart=Never --command -- sh -c "sleep 3600"
  NODE=$(kubectl get pod test-bpfmap1 -o jsonpath='{.spec.nodeName}')
  ...
  kubectl \
      run test-bpfmap \
      --privileged \
      --overrides='{"kind":"Pod", "apiVersion":"v1", "spec": {"hostNetwork":true, "nodeSelector": { "kubernetes.io/hostname": "'$NODE'" }}}' \
      --image registry.k8s.io/e2e-test-images/agnhost:2.39 \
      --restart=Never \
      --command \
      -- sh -c "
  ...
  get_icmp6_count
  get_icmp_count"
  kubectl wait --for=jsonpath='{.status.containerStatuses[0].state.terminated}' pod/test-bpfmap --timeout=300s
  output_before=$(kubectl logs test-bpfmap)
  kubectl exec -i test-bpfmap1 -- sh -c "ping -c 7 64:ff9b::$ip_address >&2"
  #gather metrics after the second ping
  sleep 20
  kubectl \
        run test-bpfmap2 \
        --privileged \
    ...
    get_icmp6_count
    get_icmp_count"
  kubectl wait --for=jsonpath='{.status.containerStatuses[0].state.terminated}' pod/test-bpfmap2 --timeout=300s
  output_after=$(kubectl logs test-bpfmap2)
  ...
  kubectl delete pod test-bpfmap1
