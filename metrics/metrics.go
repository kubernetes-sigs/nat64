...
func ReadAndUpdatePacketCount(m *ebpf.Map, c *prometheus.CounterVec) {
	...
	for key, total := range aggregatedCounters {
		...
		c.With(labels).Add(float64(total) - GetCounterValue(c, labels))
	}
}

func GetCounterValue(metric *prometheus.CounterVec, labels prometheus.Labels) float64 {
	var m = &dto.Metric{}
	if err := metric.With(labels).Write(m); err != nil {
		klog.Infof("Error accessing metrics: %v", err)
		return 0
	}
